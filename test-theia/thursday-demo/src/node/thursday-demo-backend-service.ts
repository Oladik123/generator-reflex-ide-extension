...
import {
  IAstService
} from "ast-service/lib/node/ast-service-protocol";


@injectable()
export class ThursdayDemoService implements IThursdayDemoService {

  constructor(@inject(IAstService) private readonly astService: IAstService) {}

  async getEnvVariables(): Promise < Readonly < {
    [key: string]: string | undefined
  } >> {
    return process.env;
  }

  async getText(): Promise < Readonly < {
    message: string,
    code: string
  } >> {
